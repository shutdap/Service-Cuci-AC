              ...
              <div class="row" style="padding-left: 21">
                ...
                <div class="col-sm-3" >
                  <select name="status" id="status" class="form-control">
                                ...
                                $jenisAc = mysqli_query($con,"SELECT * FROM `status`" );
                                    while ($data = mysqli_fetch_array($jenisAc)) {
                                        $status = $data['status'];
                                        $jenis = $data['keterangan'];
                                ?>
                                <option value="<?php echo $status; ?>"><?php echo $jenis;?></option>
                                <?php }?>
                            </select>
                </div>
                  <input type="submit" class="btn btn-primary" value="Cari"><span class="glyphicon glyphicon-date"></span> </a></input>
              </form>
            <!-- Light table -->
            <div class="table-responsive">
              <table class="table align-items-center table-flush">
                ...
                <tbody class="list">
                  <?php
                    require_once "../koneksi.php";
                 //    $tanggal1=$_POST['tgl_1'];
                	// $tanggal2=$_POST['tgl_2'];
                	$status=$_POST['status'];
                	$status1='';
                    ...
                    $sql = "SELECT t.*,ja.deskripsi, l.keterangan,l.jenisLayanan as layanan,t.id as tasksid,s.keterangan as status_code FROM `task` t left JOIN `jenis_ac` ja on t.jenis_ac = ja.id 
							...
							where  t.status like '%".$status1."%'";
                    $query = mysqli_query($con, $sql) or die("Gagal". mysqli_error($con));
                    while($tampil = mysqli_fetch_array($query)){
                  ?>
                  <!-- t.tgl_pesan BETWEEN '$tanggal1' and '$tanggal2' and -->
                  <tr>
                    ...
                    <?php $des = $tampil['jenis_ac'] ; if( $des != "MIXED"){ ?>
                      ...
                      <?php }else {?>
                        <td><a href="../lihatdetail.php?id=<?php echo $tampil['id'];?>" class="genric-btn success radius">Lihat Detail</a></td>
                      <?php }?>
